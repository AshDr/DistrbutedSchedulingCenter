...
template <typename T>
TCPServer<T>::TCPServer(int nServerPort, int nLengthOfQueueOfListen,
...
template <typename T> TCPServer<T>::~TCPServer() {
    if (str_boundIP_ != NULL) {
        delete[] str_boundIP_;
    }
}

template <typename T> int TCPServer<T>::Run() {
    ...
    if (::bind(nListenSocket, (sockaddr *)&ServerAddress,
    ...
    while (1) {
        ...
        std::thread client_handle_thread(&T::HandleFunction, pT, client_sock);
        client_handle_thread.detach();
    }
    ::close(nListenSocket);
    return 0;
}
template <typename T> void TCPServer<T>::HandleFunction(int client_sock) {
